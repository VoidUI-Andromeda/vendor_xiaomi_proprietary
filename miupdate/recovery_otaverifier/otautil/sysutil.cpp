...
BlockMapData BlockMapData::ParseBlockMapFile(const std::string& block_map_path) {
  ...
  if (!android::base::ReadFileToString(block_map_path, &content)) {
    // ALOGE("Failed to read %s", block_map_path.c_str());
    return {};
  }

  std::vector<std::string> lines = android::base::Split(android::base::Trim(content), "\n");
  if (lines.size() < 4) {
    // ALOGE("Block map file is too short: %d", lines.size());
    return {};
  }
  ...
  if (sscanf(lines[1].c_str(), "%" SCNu64 "%" SCNu32, &file_size, &blksize) != 2) {
    // ALOGE("Failed to parse file size and block size: %s", lines[1].c_str());
    return {};
  }

  if (file_size == 0 || blksize == 0) {
    // ALOGE("Invalid size in block map file: size %ld, blksize %d", blksize, blksize);
    return {};
  }

  size_t range_count;
  if (sscanf(lines[2].c_str(), "%zu", &range_count) != 1) {
    // ALOGE("Failed to parse block map header: %s", lines[2].c_str());
    return {};
  }
  ...
  if (blocks > std::numeric_limits<uint32_t>::max() || range_count == 0 ||
      ...
      // ALOGE("Invalid data in block map file: size %ld, blksize %d, range_count %d, lines %d",
          // file_size, blksize, range_count, lines.size());
    return {};
  }
