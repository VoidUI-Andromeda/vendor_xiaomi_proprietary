...
#define TFA2_MBDRCPARAMETER_LENGTH			(3*153)	/* 154 is including the cmd-id */ 
...
#define TFA88_FILTERCOEFSPARAMETER_LENGTH	(3*168) /*Excluding Commd ID (1 word) & Selection Type (1 word)*/
#define TFA94_FILTERCOEFSPARAMETER_LENGTH	(3*60) /* For 94/92. Excluding Commd ID (1 word) & Selection Type (1 word)*/

/* SB version 6.1.0, 6.2.0, 6.4.0 */
#define TFA72_SB_610_MBDRCPARAMETER_LENGTH			(3*99)	/* Excluding Commd ID */

/* SB version 7.0.0, 8.0.0 */
#define TFA72_SB_700_MBDRCPARAMETER_LENGTH			(3*115)	/* Excluding Commd ID */

/* SB 2.5_HP (FW 7.0.9), 3.0_HP (FW 8.4.2) specific */
#define TFA72_SB_HP_FILTERCOEFSPARAMETER_LENGTH     (3*216) /* Excluding Commd ID (1 word) & Selection Type (1 word) */
#define TFA72_SB_HP_MBDRCPARAMETER_LENGTH           (3*202) /* Excluding cmd ID Len. 203w including cmdID  */


#define TFA2_EXCURSION_FILTER_LENGTH_PER_CHANNEL	(3*25)
...
enum Tfa98xx_Error tfa98xx_dsp_mem_write(int dev_idx,
	enum Tfa98xx_DMEM which_mem, unsigned short start_offset,
...
/**
 * Function awaits DSP acknowledgement and retrievs RPC call status for message.
 *
 * @param[in] tfa TFA device structure for device that executes RPC
 * @return pRpcStatus RPC status returned by DSP
 *
 * @note This function stops execution for at least 0.6ms (busy-wait).
 * @note When DSP is still busy after 1ms, is not an error. Wait time depends on sampling frequency.
 *
 */
...
//#define TFA_FAM(tfa, fieldname) ((tfa->tfa_family == 1) ? TFA1_BF_##fieldname :  (is_94_N2_device(tfa)? TFA9894N2_BF_##fieldname : TFA2_BF_##fieldname))
...
/**
 * start the maximus hapticboost algorithm
 * this implies a full system startup when the system was not already started
 * @param tfa the device struct pointer
 * @return Tfa98xx_Error
*/
enum Tfa98xx_Error tfaRunHapticBoost(struct tfa_device *tfa);

/**
 * wait for the haptic object to finish by checking sampe count
 * @param tfa the device struct pointe
 * @return 0 if ok else timed oit
 */
int tfa_dev_hap_obj0_wait_finish(struct tfa_device *tfa );
...
/* API to get the FW API version for both DSPLess and DSP devices*/
