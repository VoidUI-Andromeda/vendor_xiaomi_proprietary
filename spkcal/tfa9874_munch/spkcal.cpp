...
#ifdef FACTORY_BUILD
#define TFA9874_CHK_PERSIST_FILE "/mnt/vendor/persist/audio/tfa9874_chk.txt"
#else
#define TFA9874_CHK_PERSIST_FILE "/data/vendor/cit/tfa9874_chk.txt"
#endif
...
#define TOP_SPEAKER_MIN_IMPEDANCE     5
#define TOP_SPEAKER_MAX_IMPEDANCE     7

#define BOT_SPEAKER_MIN_IMPEDANCE     6
#define BOT_SPEAKER_MAX_IMPEDANCE     8

double Spk_imp_min_separate[TFA98XX_MAX_DEVICES] = {TOP_SPEAKER_MIN_IMPEDANCE, BOT_SPEAKER_MIN_IMPEDANCE, Spk_imp_min, Spk_imp_min};
double Spk_imp_max_separate[TFA98XX_MAX_DEVICES] = {TOP_SPEAKER_MAX_IMPEDANCE, BOT_SPEAKER_MAX_IMPEDANCE, Spk_imp_max, Spk_imp_max};
...
int main(int argc, char *argv[])
{
		...
		do {
			...
			for (index=0; index< TFA98XX_MAX_DEVICES; index++) {
				...
				if (strlen(rdclist[index].address) != 0) {
					if ((rdclist[index].impedance > Spk_imp_min_separate[index]) && (rdclist[index].impedance < Spk_imp_max_separate[index])) {
						spkc_result = true;
					} else {
						spkc_result = false;
						break;
	...
	if (bSpkCal) {
		FILE *pFile = fopen(TFA9874_CHK_PERSIST_FILE, "ab+");
		//fprintf(pFile, "SPK: %s\n\r", boot_id);
		...
		if (access(TFA9874_CHK_PERSIST_FILE, F_OK) == 0){
			printf("TFA9874_CHK_PERSIST_FILE is exist success!\n");
			printf("\n");
			...
			printf("TFA9874_CHK_PERSIST_FILE is not exist fail!\n");
			printf("\n");
			printf("spkcal -c fail!\n");
		}
	}
	...
	if (bSpkChk) {
		...
		if (access(TFA9874_CHK_PERSIST_FILE, F_OK) == 0){
			printf("TFA9874_CHK_PERSIST_FILE is exist sucess!\n");
			printf("\n");
